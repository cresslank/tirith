...
/// Cap on a single decoded PDF stream in addition to lopdf's document-wide
/// decompression bound.
    ...
    if stream.dict.get(b"Filter").is_err() {
        if stream.content.len() > PDF_STREAM_DECODE_CAP {
            return Err(lopdf::Error::InvalidStream(
                "decoded stream exceeds the inspection budget".to_string(),
            ));
        }
...
fn pdf_named_resource<'a>(
    ...
    for resources in resources {
        let category_object = match resources.get(category) {
            Ok(object) => object,
            Err(lopdf::Error::DictKey(_)) => continue,
...
fn pdf_form_resources<'a>(
    ...
    let object = match stream.dict.get(b"Resources") {
        Ok(object) => object,
        Err(lopdf::Error::DictKey(_)) => return Ok(inherited.to_vec()),
...
fn pdf_form_matrix(doc: &lopdf::Document, stream: &lopdf::Stream) -> Result<PdfMatrix, String> {
    let object = match stream.dict.get(b"Matrix") {
        Ok(object) => object,
        Err(lopdf::Error::DictKey(_)) => return Ok(PdfMatrix::IDENTITY),
...
pub fn check_pdf(raw_bytes: &[u8]) -> Vec<Finding> {
    ...
    match pdf_structure_status(&doc) {
        ...
        PdfStructureStatus::DepthExceeded => {
            findings.push(pdf_analysis_incomplete(&[
                "decoded PDF object nesting exceeds the safe limit".to_string(),
            ]));
            return findings;
        }
        PdfStructureStatus::WorkBudgetExceeded => {
            findings.push(pdf_analysis_incomplete(&[
                "decoded PDF structure exceeds the traversal budget".to_string(),
            ]));
            return findings;
        }
    }
